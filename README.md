...
instructions at the [bottom of this page](#using-gains-as-a-template).
...
`gains/gains.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `gains.py` in directory called
`gains`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import gains as sb` in an
interactive Python session, the classes and functions defined inside of the
`gains.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the gains module, the main classes defined at the bottom of
...
Either way, you can create a `gains/data` folder in which you can
...
(typically, something like our `gains/tests/test_gains.py`). Within each
...
have one such test in `gains/tests/test_gains.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/gains`),
and type:
...
will see a message such as:
    ...
    gains/tests/test_gains.py:49: AssertionError
...
To document `gains` we use the [sphinx documentation
...
A `gains/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/gains) for the software. This
...
`gains` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `gains`, we use the
...
`gains/gains.py` to be called `smallish/smallish.py`

	git mv gains smallish
	git mv smallish/gains.py smallish/smallish.py
	git mv smallish/tests/test_gains.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `gains` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `gains` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `gains` in them. Type
the following to see where all these files are:

	git grep gains

You can replace `gains` for `smallish` quickly with:

	git grep -l 'gains' | xargs sed -i 's/gains/smallish/g'
...
Edit all the mentions of `gains` in the `gains/__init__.py` file, and in
the `gains/version.py` file as well.
